...
<p>{{class_name}}<p>
````
Kode berikut adalah kode untuk menampilkan app_name, nama_mahasiswa, dan class_name untuk aplikasi kita. Kita memakai double curly braces seperti ini __{{ ... }}__ untuk menyimpan variabel yang valuenya kita masukkan di __views.py__. Kemudian kita akan melakukan migrasi model dengan menjalankan perintah __"python manage.py makemigrations"__ diikuti __"python manage.py migrate"__ untuk melakukan migrasi model ke dalam basis data lokal. Tiap perubahan dalam model, kita harus melakukan migrasi ini agar perubahan tersebut tercatat.
...
````
Disini kita meng-_import_ module __path__ untuk mendefinisikan pola URL kita dan __show_main__ dari __main.views__ yang sudah kita buat. Kemudian pada line 4 kita menetapkan namespace __main__ sebagai konfigurasi URL ini agar tidak ada konflik dengan aplikasi berbeda. Terlihat pada line 7 nanti fungsi show_main akan dipanggil ketika URL diakses.

Setalh itu kita akan mebuka file __urls.py__ dalam direktori proyek. Kita akan mengimpor fungsi __include__ seperti ini ````from django.urls import path, include```` untuk mengimpor URL dari aplikasi main ke urls.py dalam direktori proyek. Kemudian tambahkan rute URL seperti ini:
````
urlpatterns = [
    ...
...
- MVC (Model-View_Controller) 
 - Model: Model disini berfungsi untuk mengelola dan berhubungan langsung dengan _database_
...
- MVVM(Model-View-ViewModel)
 ...
 - View: Mewakili representasi visual dai data yang ditampilkan ke pengguna
 ...
 - Perbedaan
Perbedaan yang dapat terlihat dari ketiga arsitektur ini adalah cara interkasi antara komponen-komponennya. Pada MVC kita menggunakan Controller untuk mengatur aliran aplikasi. Sedangkan MVT merupakan turunan dari struktur MVC dimana View yang berperan sebagai jembatan antara Model dan Template dan Template berfungsi sebagai tampilan untuk pengguna menggantikan peran View pada MVC. Terakhir untuk MVVM yang berperan sebagai jembatan antara Model dan View adalah ViewModelnya, sedangkan Model dan Viewnya berfungsi seperti pada MVC.